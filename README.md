...
* **v5.1 (2026-09-14)**
  * 🎯 **影视详情播放页精准守卫**：增加 `isMediaDetailPage` 页面守卫机制，仅在电影、电视剧单集详情页挂载组件；
  * 🧹 **单页路由自愈清理**：从详情页返回首页（或媒体库、分类列表）时，自动从 DOM 中彻底移除外部播放器组件，杜绝在首页（如 CCTV4K 频道卡片）残留；
  * 🏷️ **规格标签全等匹配**：对 1080P/4K/SDR 等徽章改为严格全等比对，避免任何包含 `4K` 字样的频道或影片标题被误识别为挂载点；
  * 📱 **紧凑单行防折行设计 (v5.0 继承)**：全套播放器按钮尺寸缩小 40%，强制 `nowrap` 单行平铺，高度视觉对齐。
* **v4.9 (2026-09-14)**
