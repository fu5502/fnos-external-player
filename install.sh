...
# 导出 window.__ug (动态匹配包含 new ic 的主逻辑文件，彻底摆脱 Vite 打包 hash 变动失效问题)
ASSET_JS=$(grep -l "new ic(" /usr/local/apps/@appcenter/trim.media/static/assets/*.js 2>/dev/null | head -n 1 || true)
if [ -n "$ASSET_JS" ] && [ -f "$ASSET_JS" ] && ! grep -q "window.__ug=" "$ASSET_JS"; then
...
if [ -f "$HTML1" ]; then
    if grep -q "fnExternalPlayer.js" "$HTML1"; then
        sed -i 's/fnExternalPlayer\.js?v=[0-9.]*/fnExternalPlayer.js?v=5.2.3/g' "$HTML1"
    else
        sed -i 's/<\/body>/<script src="\/v\/static\/fnExternalPlayer.js?v=5.2.3" defer><\/script><\/body>/g' "$HTML1"
    fi
fi
...
if [ -f "$HTML2" ]; then
    if grep -q "fnExternalPlayer.js" "$HTML2"; then
        sed -i 's/fnExternalPlayer\.js?v=[0-9.]*/fnExternalPlayer.js?v=5.2.3/g' "$HTML2"
    else
        sed -i 's/<\/body>/<script src="\/static\/fnExternalPlayer.js?v=5.2.3" defer><\/script><\/body>/g' "$HTML2"
    fi
fi
...
cat << 'EOF' > /etc/cron.d/fn_player_daemon
*/10 * * * * root /usr/local/bin/fn_player_daemon.sh >/dev/null 2>&1
@reboot root /usr/local/bin/fn_player_daemon.sh >/dev/null 2>&1
EOF
chmod 644 /etc/cron.d/fn_player_daemon
