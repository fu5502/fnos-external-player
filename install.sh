...
if [ -f "$HTML1" ]; then
    if grep -q "fnExternalPlayer.js" "$HTML1"; then
        sed -i 's/fnExternalPlayer\.js?v=[0-9.]*/fnExternalPlayer.js?v=5.2/g' "$HTML1"
    else
        sed -i 's/<\/body>/<script src="\/v\/static\/fnExternalPlayer.js?v=5.2" defer><\/script><\/body>/g' "$HTML1"
    fi
fi
...
if [ -f "$HTML2" ]; then
    if grep -q "fnExternalPlayer.js" "$HTML2"; then
        sed -i 's/fnExternalPlayer\.js?v=[0-9.]*/fnExternalPlayer.js?v=5.2/g' "$HTML2"
    else
        sed -i 's/<\/body>/<script src="\/static\/fnExternalPlayer.js?v=5.2" defer><\/script><\/body>/g' "$HTML2"
    fi
fi
